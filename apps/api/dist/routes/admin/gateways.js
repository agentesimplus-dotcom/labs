...
exports.default = adminGatewaysRoutes;
const shared_1 = require("@esl/shared");
async function adminGatewaysRoutes(fastify) {
    ...
    fastify.get('/admin/gateways', {
        ...
        const [data, total] = await Promise.all([
            shared_1.prisma.gateway.findMany({
                where,
            ...
            shared_1.prisma.gateway.count({ where })
        ]);
        const formatted = data.map((gw) => ({
            ...gw,
            macDisplay: (0, shared_1.formatMacForDisplay)(gw.macAddress),
    ...
    fastify.post('/admin/gateways', {
        ...
        if (!(0, shared_1.isValidMac)(body.macAddress)) {
        ...
        const macNorm = (0, shared_1.normalizeMac)(body.macAddress);
        // Verify store belongs to tenant
        const store = await shared_1.prisma.store.findFirst({ where: { id: body.storeId, tenantId } });
        if (!store)
            return reply.status(400).send({ error: 'Store not found or does not belong to this tenant.' });
        // Check duplicate
        const existing = await shared_1.prisma.gateway.findUnique({ where: { macAddress: macNorm } });
        if (existing)
            return reply.status(409).send({ error: 'A gateway with this MAC address already exists.' });
        const gateway = await shared_1.prisma.gateway.create({
            data: {
                macAddress: macNorm,
                tenantId,
        ...
        return { ...gateway, macDisplay: (0, shared_1.formatMacForDisplay)(gateway.macAddress), lastSeq: gateway.lastSeq?.toString() };
    });
    ...
    fastify.put('/admin/gateways/:mac', {
        ...
        const body = request.body;
        const gateway = await shared_1.prisma.gateway.findUnique({ where: { macAddress: macNorm } });
        ...
        const updated = await shared_1.prisma.gateway.update({ where: { macAddress: macNorm }, data });
    ...
    fastify.delete('/admin/gateways/:mac', {
        ...
        await shared_1.prisma.gateway.delete({ where: { macAddress: macNorm } });
