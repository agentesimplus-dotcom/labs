...
describe("pack_to_hex", () => {
    ...
    it("packs an 8x1 image LSB-first correctly", () => {
        ...
        const pixels = [1, 0, 1, 0, 1, 0, 1, 1];
        const hex = pack_to_hex(pixels, 8, 1, { endianness: "little" });
        expect(hex).toBe("D5");
    });
    ...
    it("handles width not multiple of 8 (row padding)", () => {
        ...
        const hex = pack_to_hex(pixels, 10, 2, { endianness: "big" });
    ...
    it("inverts pixels when invert options is passed", () => {
        ...
        const hex = pack_to_hex(pixels, 8, 1, { endianness: "big", invert: true });
        expect(hex).toBe("FF");
    });
});
