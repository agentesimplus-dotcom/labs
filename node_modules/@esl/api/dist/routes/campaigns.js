...
async function campaignRoutes(fastify) {
    ...
    fastify.get('/campaigns', {
        ...
        return shared_1.prisma.campaign.findMany({
            where: { tenantId },
            include: {
    ...
    fastify.post('/campaigns', {
        ...
        const { tenantId } = request.user;
        const { name, storeId, startAt, endAt, templateVersionId, filterJson } = request.body;
        // Validate template version
        const tv = await shared_1.prisma.templateVersion.findUnique({
            where: { id: templateVersionId }, include: { template: true }
        });
        if (!tv || tv.template.tenantId !== tenantId)
            return reply.code(404).send({ error: 'Template version not found' });
        if (!tv.isPublished)
            return reply.code(400).send({ error: 'Template version must be published' });
        const campaign = await shared_1.prisma.campaign.create({
    ...
    fastify.post('/campaigns/:id/:action', {
        ...
        const { tenantId } = request.user;
        const campaign = await shared_1.prisma.campaign.findFirst({ where: { id, tenantId } });
        if (!campaign)
            return reply.code(404).send({ error: 'Campaign not found' });
        const newStatus = action === 'activate' ? 'ACTIVE' : 'PAUSED';
        if (campaign.status !== newStatus) {
            await shared_1.prisma.campaign.update({ where: { id }, data: { status: newStatus } });
        }
    ...
    fastify.get('/campaigns/:id/impact', {
        ...
        if (campaign.filterJson) {
            ...
            if (parsed.skus?.length)
                filter.productId = { in: parsed.skus };
        }
        if (campaign.storeId)
            filter.storeId = campaign.storeId;
        filter.tenantId = tenantId;
        const count = await shared_1.prisma.tag.count({ where: filter });
    ...
    fastify.post('/campaigns/:id/push', {
        ...
        const tagFilter = { tenantId, productId: { in: skus }, status: 'ACTIVE' };
        if (campaign.storeId)
        ...
        const assignmentFilter = { tenantId, sku: { in: skus } };
        if (campaign.storeId)
            assignmentFilter.storeId = campaign.storeId;
        const result = await shared_1.prisma.tagAssignment.updateMany({
            where: assignmentFilter,
            data: { templateVersionId: campaign.templateVersionId }
        });
        // Optionally, ensure campaign status is ACTIVE
        if (campaign.status !== 'ACTIVE') {
            await shared_1.prisma.campaign.update({ where: { id }, data: { status: 'ACTIVE' } });
        }
        // Trigger rendering jobs
        const assignments = await shared_1.prisma.tagAssignment.findMany({
            where: assignmentFilter,
            include: { tag: true }
        });
        let queuedCount = 0;
    ...
    fastify.delete('/campaigns/:id', {
        ...
        if (!campaign)
            return reply.code(404).send({ error: 'Campaign not found' });
        await shared_1.prisma.campaign.delete({ where: { id } });
